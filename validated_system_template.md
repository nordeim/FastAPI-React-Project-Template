https://github.com/copilot/share/c05a1236-4b20-8c03-a842-f048e03349cb

````markdown name=validated-system-prompt.md
...
I reviewed your original template line-by-line, preserved its structure and intent, and tightened ambiguous behaviors (repo access, secrets, large file handling, env parsing, startup side-effects). I also added explicit utility snippets (ALLOWED_ORIGINS parsing, Vite API base env usage), failure-mode steps for committed DB files, and a short "final checklist" to guarantee a repeatable, auditable output from the LLM. If you want, I will now convert this validated prompt into a ready-to-paste System prompt (single-line-friendly), or generate a PR-ready patch set for the high-priority repo fixes I identified earlier (remove DB, .gitignore updates, .env.example, API_BASE_URL change, safe startup). Which should I do next?
